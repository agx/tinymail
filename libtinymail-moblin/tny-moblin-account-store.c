...
typedef struct _TnyMoblinAccountStorePriv TnyMoblinAccountStorePriv;

struct _TnyMoblinAccountStorePriv
{
	GConfClient *client;
	gchar *cache_dir;
...
#define TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE(o)	\
	(G_TYPE_INSTANCE_GET_PRIVATE ((o), TNY_TYPE_MOBLIN_ACCOUNT_STORE, TnyMoblinAccountStorePriv))


static gchar*
per_account_get_pass(TnyAccount *account, const gchar *prompt, gboolean *cancel)
{
	TnyPlatformFactory *platfact = tny_moblin_platform_factory_get_instance ();
...
static gboolean
tny_moblin_account_store_alert (TnyAccountStore *self, TnyAccount *account, TnyAlertType type, gboolean question, GError *error)
{
	...
	dialog = gtk_message_dialog_new (NULL, GTK_DIALOG_MODAL,
		gtktype, GTK_BUTTONS_YES_NO, "%s", error->message);
...
static void
kill_stored_accounts (TnyMoblinAccountStorePriv *priv)
{
	if (priv->accounts)
	{
			...
			GConfEntry *entry, gpointer user_data)
{
	...
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static TnyAccount*
tny_moblin_account_store_find_account (TnyAccountStore *self, const gchar *url_string)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static const gchar*
tny_moblin_account_store_get_cache_dir (TnyAccountStore *self)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static void
tny_moblin_account_store_get_accounts (TnyAccountStore *self, TnyList *list, TnyGetAccountsRequestType types)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static void
tny_moblin_account_store_notify_add (TnyAccountStore *self)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static void
tny_moblin_account_store_notify_remove (TnyAccountStore *self)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static TnyDevice*
tny_moblin_account_store_get_device (TnyAccountStore *self)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
/**
 * tny_moblin_account_store_new:
 *
 * Create a #TnyAccountStore for MOBLIN desktops that uses GConf for storing
 * the account information and detecting changes to that configuration.
 *
 * Return value: A new #TnyAccountStore instance
 **/
TnyAccountStore*
tny_moblin_account_store_new (void)
{
	TnyMoblinAccountStore *self = g_object_new (TNY_TYPE_MOBLIN_ACCOUNT_STORE, NULL);
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
...
static void
tny_moblin_account_store_instance_init (GTypeInstance *instance, gpointer g_class)
{
	TnyMoblinAccountStore *self = (TnyMoblinAccountStore *)instance;
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
	...
	tny_moblin_account_store_notify_add (TNY_ACCOUNT_STORE (self));

	platfact = TNY_PLATFORM_FACTORY (tny_moblin_platform_factory_get_instance ());
...
static void
tny_moblin_account_store_finalize (GObject *object)
{
	TnyMoblinAccountStore *self = (TnyMoblinAccountStore *)object;
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);

	tny_moblin_account_store_notify_remove (TNY_ACCOUNT_STORE (self));
...
/**
 * tny_moblin_account_store_get_session:
 * @self: The #TnyMoblinAccountStore instance
 *
 ...
 **/
TnySessionCamel*
tny_moblin_account_store_get_session (TnyMoblinAccountStore *self)
{
	TnyMoblinAccountStorePriv *priv = TNY_MOBLIN_ACCOUNT_STORE_GET_PRIVATE (self);
	return priv->session;
}
...
static void
tny_moblin_account_store_class_init (TnyMoblinAccountStoreClass *class)
{
	...
	object_class->finalize = tny_moblin_account_store_finalize;

	g_type_class_add_private (object_class, sizeof (TnyMoblinAccountStorePriv));
...
tny_account_store_init (gpointer g, gpointer iface_data)
{
	...
	klass->get_accounts= tny_moblin_account_store_get_accounts;
	klass->get_cache_dir= tny_moblin_account_store_get_cache_dir;
	klass->get_device= tny_moblin_account_store_get_device;
	klass->alert= tny_moblin_account_store_alert;
	klass->find_account= tny_moblin_account_store_find_account;

	return;
}

static gpointer
tny_moblin_account_store_register_type (gpointer notused)
{
	GType type = 0;

	static const GTypeInfo info =
		{
			sizeof (TnyMoblinAccountStoreClass),
			NULL,   /* base_init */
			NULL,   /* base_finalize */
			(GClassInitFunc) tny_moblin_account_store_class_init,   /* class_init */
			NULL,   /* class_finalize */
			NULL,   /* class_data */
			sizeof (TnyMoblinAccountStore),
			0,      /* n_preallocs */
			tny_moblin_account_store_instance_init    /* instance_init */
		};
...
/**
 * tny_moblin_account_store_get_type:
 *
 ...
 **/
GType
tny_moblin_account_store_get_type (void)
{
