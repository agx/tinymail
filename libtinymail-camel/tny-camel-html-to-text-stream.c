...
tny_camel_html_to_text_stream_new (TnyStream *html_stream)
{
	...
	mime_filter = (CamelMimeFilter *) camel_mime_filter_html_new ();

	priv->stream = (CamelStream *) camel_stream_filter_new_with_stream (wrap_html_stream);
	camel_stream_filter_add ((CamelStreamFilter *) priv->stream, mime_filter);
